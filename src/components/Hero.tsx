...
const Hero = () => {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      const diff = 6 - day >= 0 ? 6 - day : 6 - day + 7; // days until next Saturday
      nextSaturday.setDate(now.getDate() + diff);
      nextSaturday.setHours(17,0,0,0); // Saturday 5 PM
...
>
  ...
  <div className="container mt-12 relative z-10 flex flex-col md:flex-row items-center gap-10 px-4 md:px-0">
    {/* Left Side: Main Text */}
    <div className="md:w-2/3 max-w-4xl text-center md:text-left order-1 md:order-1">
    ...
    {/* Right Side: Announcement Column */}
    <div className="md:w-1/3 w-full flex flex-col items-center mb-5 md:items-center bg-white/20 backdrop-blur-md rounded-xl p-6 space-y-4 shadow-lg order-2 md:order-2">
      ...
      <a
        href="https://docs.google.com/forms/" //Sanjay replace this with actual link here
        target="_blank"
